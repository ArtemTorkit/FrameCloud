...
router.route('/get-ai-link').get(async (req, res) => {
    result = await pool.query(`
            SELECT *
            FROM ailink
            WHERE id = 1
        `)
    console.log('link', result[0])
    res.json({ message: result[0][0].link })
})

router.route('/change-link').post(async (req, res) => {
    const { link } = req.body;
