...
const ProfileGuest = () => {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:5000/post/get-userphotos-byid', {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({id:idUser})
    ...
    const loadMoreContent = () => {
        fetch('http://localhost:5000/post/get-userposts-byid', {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ lastPostId, id: idUser })
        }).then(response => response.json())
            .then(data => {
    ...
    const handleSubscribe = async () => {
        try {
            const response = await fetch('http://localhost:5000/user/subscribe', {
                ...
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ idCreator: idUser })
            });
            if (!response.ok) {
